...
import { DUMMY_INVENTORY_ITEMS } from "@/app/inventory/items/dummyData";
import type { AvailableItemRow, RequestLineItem } from "./types";

// Derive available items from inventory: product name (general), quantity, uom
function useAvailableItems(): AvailableItemRow[] {
  return useMemo(
    () =>
      DUMMY_INVENTORY_ITEMS.map((item) => ({
        id: item.id,
...
export default function StoreRequestItemPage() {
  const available = useAvailableItems();
  const [cart, setCart] = useState<RequestLineItem[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalItem, setModalItem] = useState<AvailableItemRow | null>(null);
  const [formProductName, setFormProductName] = useState("");
  const [formQuantity, setFormQuantity] = useState<string>("1");
  const [formUom, setFormUom] = useState("");

  const openModal = (row: AvailableItemRow) => {
    setModalItem(row);
    setFormProductName(row.productNameGeneral);
    setFormQuantity("1");
    setFormUom(row.measurement);
    setModalOpen(true);
  };
  ...
  const addToCart = () => {
    ...
    const line: RequestLineItem = {
      ...
      quantity: qty,
      unitOfMeasurement: (formUom.trim() || modalItem?.measurement) ?? "",
    };
