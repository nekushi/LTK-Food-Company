...
import type {
  POSReceiptGroup,
  POSInventoryItem,
} from "@/dal/store/pos-receipts";
import {
  sendPOSDailyReport,
  getPOSReceiptsByDate,
} from "@/dal/store/pos-receipts";
...
export default function StoreSalesReportClient({
  ...
  const [invAccountRecognition, setInvAccountRecognition] = useState(
    ACCOUNT_RECOGNITIONS[0],
  );
  ...
  const [invFormMonth, setInvFormMonth] = useState(
    MONTHS[new Date().getMonth()],
  );
  const [invFormYear, setInvFormYear] = useState(
  ...
  const [sendingInvReport, setSendingInvReport] = useState(false);
  ...
  const [filteredReceipts, setFilteredReceipts] =
    useState<POSReceiptGroup[]>(posReceipts);
  const [filteredReceiptsTotal, setFilteredReceiptsTotal] =
    useState(posTotalSales);
  ...
  const handleSendInventoryReport = async () => {
    if (inventoryReports.length === 0) {
      toast.error("No inventory reports to send");
      return;
    }
    setSendingInvReport(true);
    const uid = getAuth("userId") || "";
    const reportDate = new Date().toISOString().split("T")[0];
    const reports = inventoryReports.map((r) => ({
      id: r.id,
    ...
    const result = await sendPOSDailyReport(uid, "inventory_report", {
      reportDate,
      reports,
    });
    if (result.success) toast.success("Inventory report sent to admin");
    else toast.error(result.message);
    setSendingInvReport(false);
  };
  ...
  const handleInventorySubmit = async (e: React.FormEvent) => {
    ...
    if (invReportType === "Daily" && invBatchCart.length > 0) {
      ...
      if (saved.length) {
        ...
        toast.success(
          `Saved ${saved.length} daily report(s).${failed ? ` ${failed} failed.` : ""}`,
        );
      }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 shadow-sm h-fit">
                ...
                <p className="mb-4 text-xs text-amber-700/80">
                  Only daily reports are submitted. Weekly and monthly totals
                  are generated from your daily entries.
                </p>
              ...
              <div className="overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm flex flex-col h-[600px]">
                ...
                <div className="overflow-y-auto flex-1 p-0">
                  {reports.filter((r) => r.reportType === "Daily").length ===
                  0 ? (
                          ...
                          .map((report) => (
                            ...
                            >
                              ...
                              <td className="px-6 py-3 text-emerald-700 font-bold text-right">
                                ₱
                                {Number(report.totalSales).toLocaleString(
                                  undefined,
                                  {
          ...
          <div className="rounded-xl border border-amber-200 bg-white shadow-sm overflow-hidden">
            <div className="border-b border-amber-200 bg-amber-50 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div>
                  <label className="sr-only" htmlFor="receipt-date-filter">
                    Filter by date
                  </label>
                  <input
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-amber-50">
                    {filteredReceipts.map((receipt) =>
                      receipt.lines.map((line, idx) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 text-amber-900 font-medium">
                            {line.itemName}
                          </td>
                          <td className="px-5 py-3 text-amber-800 font-bold text-center">
                            {line.quantity}
                          </td>
                    ...
                    <tr className="bg-amber-50 border-t-2 border-amber-300">
                      <td
                        colSpan={6}
                        className="px-5 py-3 text-sm font-bold text-amber-900 text-right"
                      >
                        Grand Total
                      </td>
                      <td className="px-5 py-3 text-lg font-bold text-emerald-700 text-right">
                        ₱
                        {filteredReceiptsTotal.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-1 gap-8">
            <Activity mode="hidden">
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 shadow-sm h-fit">
                ...
                {invReportType === "Daily" && (
                  <p className="mb-4 text-xs text-amber-700/80">
                    Add multiple products for the same day to the batch, then
                    submit once.
                  </p>
                )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setInvAccountRecognition(e.target.value)
                        }
                  ...
                  <div>
                    ...
                    {isProductFromInventory && (
                      <p className="mt-1 text-xs text-amber-600/80">
                        From branch inventory (0 is allowed)
                      </p>
                    )}
                  </div>
                    ...
                    <div className="mt-4 space-y-3">
                      <button
                        ...
                        onClick={() => {
                          ...
                          const periodYear = new Date(invFormDate)
                            .getFullYear()
                            .toString();
                        ...
                        <>
                          <div className="rounded-lg border border-amber-200 bg-white max-h-40 overflow-y-auto">
                            <div className="px-3 py-2 border-b border-amber-100 text-xs font-semibold text-amber-800">
                              Batch — {invBatchCart.length} item(s) for{" "}
                              {new Date(
                                invBatchCart[0].periodMonth,
                          ...
                          >
                            {invSubmitting
                              ? "Submitting..."
                              : `Submit batch (${invBatchCart.length} items)`}
                          </button>
                        </>
                      )}
                    </div>
            ...
            <div className="overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm flex flex-col h-[600px]">
              <div className="border-b border-amber-200 bg-amber-50 px-4 py-3 shrink-0 flex items-center justify-between gap-3">
                <h2 className="text-sm font-semibold text-amber-900">
                  Submitted Inventory Reports
                </h2>
                <button
                  type="button"
                  onClick={handleSendInventoryReport}
                  disabled={sendingInvReport || inventoryReports.length === 0}
                ...
                >
                  <FiSend className="text-sm" />
                  {sendingInvReport ? "Sending..." : "Send Report"}
                </button>
              </div>
              <div className="overflow-y-auto flex-1 p-0">
              ...
              <div className="border-b border-amber-200 bg-amber-50 px-6 py-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-amber-600 mt-0.5">
                    Tracks initial stock, sold quantities, and remaining stock
                    for items issued today.
                  </p>
                </div>
                <button
                    ...
                    <tbody className="divide-y divide-amber-50">
                      {posInventory.map((item) => {
                        const pct =
                          item.initialStock > 0
                            ? Math.round(
                                (item.remainingStock / item.initialStock) * 100,
                              )
                            : 0;
                        return (
                          <tr
                          ...
                          >
                            ...
                            <td className="px-5 py-3 text-amber-700 font-bold text-right">
                              {item.initialStock}
                            </td>
