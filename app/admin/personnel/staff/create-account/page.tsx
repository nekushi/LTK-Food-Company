  ...
  return (
    <div className="space-y-6 p-8">
      <button className="underline underline-offset cursor-pointer mb-2 hover:text-yellow-600 block">
        <Link href={"/admin/personnel/staff"} className="flex items-center gap-1">
          <FaArrowLeft className="text-xs underline" />
          back to staff personnel
        </Link>
      </button>
      <CreateAccountForm allowedRoles={["INVENTORY", "DELIVERY"]} redirectPath="/admin/personnel/staff" />
    </div>
  );
}
