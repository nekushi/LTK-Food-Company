...
export default function CustomReportClient({ stores }: { stores: Store[] }) {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html>
        <head>
          <title>Report — ${selectedStoreName}</title>
          <style>
    ...
    <div className="p-8 space-y-6">
        ...
        <div className="space-y-5">
          ...
          {reportType === "sales" ? (
            ...
            <div>
              ...
              <select
                value={invRange}
                onChange={(e) => { setInvRange(e.target.value as "daily" | "past7days"); setGenerated(false); }}
                className="w-full rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm text-amber-900 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
              >
      ...
      {generated && (
        <div className="rounded-xl border border-amber-200 bg-white p-6 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-amber-600">
                {reportType === "inventory"
                  ? invRange === "daily" ? "Today" : "Past 7 days"
          ...
          <div ref={printRef}>
            <h1 style={{ display: "none" }}>{activeLabel} — {selectedStoreName}</h1>
            <p className="meta" style={{ display: "none" }}>{reportType === "inventory" ? (invRange === "daily" ? "Today" : "Past 7 days") : `${selectedMonth === "All" ? "All months" : selectedMonth} ${selectedYear}`}</p>

            {reportType === "sales" && (
              <div className="space-y-8">
                {/* Daily Sales Table */}
                <div>
                  <h2 className="text-sm font-semibold text-amber-800 mb-3 bg-amber-50 px-3 py-1.5 rounded-md border border-amber-100">
                    Daily Sales
                  </h2>
                      ...
                      <table className="w-full text-left text-sm">
                        <thead className="border-b border-amber-200 text-amber-900">
                          <tr>
                            ...
                            <th className="py-2 px-2 font-semibold text-right">Total Sales</th>
                          </tr>
                        </thead>
                          ...
                          {dailySales.map((r) => (
                            <tr key={r.id} className="hover:bg-amber-50/40">
                              ...
                              <td className="py-2 px-2 text-right font-medium text-emerald-700">
                                ₱{r.totalSales.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot className="border-t border-amber-300">
                          <tr>
                            <td className="py-2 px-2 font-semibold text-amber-900">Total</td>
                            <td className="py-2 px-2 text-right font-bold text-emerald-700">
                              ₱{dailySales.reduce((sum, r) => sum + r.totalSales, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
                ...
                <div>
                  <h2 className="text-sm font-semibold text-amber-800 mb-3 bg-amber-50 px-3 py-1.5 rounded-md border border-amber-100">
                    Weekly Sales <span className="font-normal text-amber-600 text-[11px] ml-1">(auto-calculated, Sunday start)</span>
                  </h2>
                  <div className="overflow-x-auto">
                    {computedWeekly.length > 0 ? (
                      <table className="w-full text-left text-sm">
                        <thead className="border-b border-amber-200 text-amber-900">
                          <tr>
                            <th className="py-2 px-2 font-semibold">Week</th>
                            <th className="py-2 px-2 font-semibold text-right">Total Sales</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-amber-100">
                          {computedWeekly.map((w) => (
                            <tr key={w.label} className="hover:bg-amber-50/40">
                              <td className="py-2 px-2 text-amber-800 text-xs">{w.label}</td>
                              <td className="py-2 px-2 text-right font-medium text-emerald-700">
                                ₱{w.totalSales.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot className="border-t border-amber-300">
                          <tr>
                            <td className="py-2 px-2 font-semibold text-amber-900">Total</td>
                            <td className="py-2 px-2 text-right font-bold text-emerald-700">
                              ₱{computedWeekly.reduce((sum, w) => sum + w.totalSales, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
                  ...
                  <div className="overflow-x-auto">
                    {computedMonthly.length > 0 ? (
                      <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-amber-100">
                          {computedMonthly.map((m) => (
                            <tr key={m.label} className="hover:bg-amber-50/40">
                              <td className="py-2 px-2 text-amber-800">{m.label}</td>
                              <td className="py-2 px-2 text-right font-medium text-emerald-700">
                                ₱{m.totalSales.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot className="border-t border-amber-300">
                          <tr>
                            <td className="py-2 px-2 font-semibold text-amber-900">Total</td>
                            <td className="py-2 px-2 text-right font-bold text-emerald-700">
                              ₱{computedMonthly.reduce((sum, m) => sum + m.totalSales, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
