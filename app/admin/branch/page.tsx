    ...
    <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {success && metrics && metrics.length > 0 ? (
            metrics.map((metric) => (
              <div
                key={metric.storeId}
                className="rounded-xl border border-amber-200 bg-white p-6 shadow-sm flex flex-col"
              >
                <h3
                  className="text-sm font-semibold text-amber-700 mb-1 truncate"
                  title={metric.storeName}
                >
                  {metric.storeName}
                </h3>
                  ...
                  <>
                    <div className="mt-2 flex items-baseline gap-2">
                      <span className="text-2xl font-bold text-amber-900 truncate">
                        ₱
                        {metric.latestSales.toLocaleString(undefined, {
      ...
      <div>
        ...
        <p className="text-amber-700">
          Content for Branch List and Branch Profile will go here.
        </p>
      </div>
    </div>
  );
