...
import {
  FiArrowLeft, FiUser, FiShield, FiBriefcase, FiPhone, FiMail,
  FiMapPin, FiCalendar, FiHash, FiEdit2, FiX, FiSave,
} from "react-icons/fi";
...
export default function ProfileClient({ account, employeeData, employeeId }: ProfileClientProps) {
  ...
  const dateHired = employeeData?.dateHired
    ? new Date(employeeData.dateHired).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
    : null;

  return (
    <div className="p-8 space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <Link href="/hr/accounts" className="flex items-center gap-2 text-sm text-amber-700 hover:text-amber-900 transition-colors">
          ...
          <div className="flex-1 min-w-0">
            {editing ? (
              <div className="grid grid-cols-2 gap-3 max-w-md">
                ...
                <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" className={`${inputClass} col-span-2`} />
              </div>
            ) : (
              <>
            ...
            <div className="mt-2 flex items-center gap-3">
              <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full ${roleStyle.bg} ${roleStyle.text}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-amber-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-3">
            <InfoRow label="Date Hired" value={dateHired} icon={<FiCalendar className="text-amber-500" />} />
            {editing ? (
              <>
                <EditRow label="SSS" value={sss} onChange={setSss} icon={<FiShield className="text-amber-500" />} />
                <EditRow label="Pag-IBIG" value={pagIbig} onChange={setPagIbig} icon={<FiShield className="text-amber-500" />} />
                <EditRow label="PhilHealth" value={philhealth} onChange={setPhilhealth} icon={<FiShield className="text-amber-500" />} />
                <EditRow label="TIN" value={tin} onChange={setTin} icon={<FiShield className="text-amber-500" />} />
              </>
            ) : (
              <>
                <InfoRow label="SSS" value={sss} icon={<FiShield className="text-amber-500" />} />
                <InfoRow label="Pag-IBIG" value={pagIbig} icon={<FiShield className="text-amber-500" />} />
                <InfoRow label="PhilHealth" value={philhealth} icon={<FiShield className="text-amber-500" />} />
                <InfoRow label="TIN" value={tin} icon={<FiShield className="text-amber-500" />} />
              </>
            )}
          </div>
        </div>
      ...
      <div className="rounded-xl border border-amber-200 bg-white p-6 shadow-sm">
        ...
        {editing ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <ContactEditCard label="Phone" icon={<FiPhone className="text-amber-600" />} value={contactNo} onChange={setContactNo} />
            <ContactEditCard label="Email" icon={<FiMail className="text-amber-600" />} value={email} onChange={setEmail} />
            <ContactEditCard label="Address" icon={<FiMapPin className="text-amber-600" />} value={address} onChange={setAddress} />
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <ContactCard label="Phone" icon={<FiPhone className="text-amber-600" />} value={contactNo} />
            <ContactCard label="Email" icon={<FiMail className="text-amber-600" />} value={email} />
            <ContactCard label="Address" icon={<FiMapPin className="text-amber-600" />} value={address} />
          </div>
        )}
      </div>
    </div>
...
function InfoRow({ label, value, icon }: { label: string; value: string | null | undefined; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-amber-50 last:border-b-0">
      <span className="text-xs text-amber-600 flex items-center gap-1.5">{icon}{label}</span>
      <span className={`text-sm font-medium ${value ? "text-amber-900" : "text-amber-400 italic"}`}>
        {value || "N/A"}
      </span>
    </div>
  );
...
function EditRow({ label, value, onChange, icon }: { label: string; value: string; onChange: (v: string) => void; icon?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-4 py-1.5 border-b border-amber-50 last:border-b-0">
      <span className="text-xs text-amber-600 flex items-center gap-1.5 shrink-0">{icon}{label}</span>
      <input
        value={value}
...
function ContactCard({ label, icon, value }: { label: string; icon: React.ReactNode; value: string }) {
  return (
    <div className="flex items-start gap-3 rounded-lg bg-amber-50/50 border border-amber-100 p-4">
      <span className="mt-0.5 shrink-0">{icon}</span>
      <div>
        <p className="text-[11px] text-amber-600 font-medium uppercase tracking-wide">{label}</p>
        <p className={`text-sm font-medium mt-0.5 ${value ? "text-amber-900" : "text-amber-400 italic"}`}>
          {value || "N/A"}
        </p>
      </div>
    </div>
...
function ContactEditCard({ label, icon, value, onChange }: { label: string; icon: React.ReactNode; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex items-start gap-3 rounded-lg bg-amber-50/50 border border-amber-100 p-4">
      <span className="mt-0.5 shrink-0">{icon}</span>
      <div className="flex-1">
        <p className="text-[11px] text-amber-600 font-medium uppercase tracking-wide mb-1">{label}</p>
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder="N/A"
